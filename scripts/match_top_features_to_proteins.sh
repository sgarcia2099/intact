...
NR > 1 {
  ...
  qs = "."
  iso = "."
  if ("quality_score" in idx) qs = $(idx["quality_score"])
  if ("isotope_cosine" in idx) iso = $(idx["isotope_cosine"])
  ...
  print sample_id, feature_id, neutral_mass + 0, intensity + 0, rt + 0, qs, iso
}
' "${FEATURES_FILE}" \
